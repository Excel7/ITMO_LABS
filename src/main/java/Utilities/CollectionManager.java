...
public class CollectionManager {
    ...
    public static String info() {
        String response = "";
        response += "Collection's type: " + listProduct.getClass().getSimpleName() + '\n';
    ...
    public static String addIfMax(Product o) {
        boolean judge = true;
        for (Product product : listProduct) {
            if (o.compareTo(product) < 0) {
                judge = false;
                System.out.println("Product not added because it is not max");
                break;
            }
        }
        if (judge) {
            listProduct.add(o);
            System.out.println("AddIfMax command successful");
        }
        return "Product added";
    }

    public static String addIfMin(Product o) {
        boolean judge = true;
        for (Product product : listProduct) {
            if (o.compareTo(product) > 0) {
                judge = false;
                System.out.println("Product not added because it is not min");
                break;
            }
        }
        if (judge) {
            listProduct.add(o);
            System.out.println("AddIfMin command successful");

        }
        return "Product added";
    }

    public static String filterContainsName(String nameString) {
        String ans = null;
        for (Product product : listProduct) {
    ...
    public static String  countByManufacturer(String nameString) {
        Integer counter = 0;
        String ans = null;
        for (Product product : listProduct) {
